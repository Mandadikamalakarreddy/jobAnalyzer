    ...
    <div className={cn("space-y-6", className)}>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">{analysis.jobTitle}</h1>
            <p className="text-lg text-gray-600">{analysis.company}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            <div className="text-2xl font-bold text-blue-600">{jobAnalysis.requiredSkills.length}</div>
            <div className="text-sm text-blue-800">Required Skills</div>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <div className="text-2xl font-bold text-green-600">{jobAnalysis.preferredSkills.length}</div>
            <div className="text-sm text-green-800">Preferred Skills</div>
          </div>
          <div className="text-center p-4 bg-purple-50 rounded-lg">
            <div className="text-2xl font-bold text-purple-600">{jobAnalysis.technicalStack.length}</div>
            <div className="text-sm text-purple-800">Tech Stack Items</div>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-2">
            {jobAnalysis.requiredSkills.map((skill, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-100">
                ...
                <span className="text-xs text-red-600 bg-red-200 px-2 py-1 rounded-full">Required</span>
              </div>
            ))}
            {jobAnalysis.requiredSkills.length === 0 && (
              <p className="text-gray-500 italic">No specific required skills identified</p>
            )}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {jobAnalysis.preferredSkills.map((skill, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-100">
                ...
                <span className="text-xs text-green-600 bg-green-200 px-2 py-1 rounded-full">Nice to have</span>
              </div>
            ))}
            {jobAnalysis.preferredSkills.length === 0 && (
              <p className="text-gray-500 italic">No specific preferred skills identified</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {jobAnalysis.technicalStack.length === 0 && (
            <p className="text-gray-500 italic">No specific technical stack identified</p>
          )}
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {jobAnalysis.companyInfo.size && (
              <div className="text-center p-4 bg-orange-50 rounded-lg border border-orange-100">
                <div className="font-semibold text-orange-800 capitalize">{jobAnalysis.companyInfo.size}</div>
                <div className="text-sm text-orange-600">Company Size</div>
              </div>
            )}
            {jobAnalysis.companyInfo.industry && (
              <div className="text-center p-4 bg-orange-50 rounded-lg border border-orange-100">
                <div className="font-semibold text-orange-800">{jobAnalysis.companyInfo.industry}</div>
            ...
            {jobAnalysis.companyInfo.culture && jobAnalysis.companyInfo.culture.length > 0 && (
              <div className="p-4 bg-orange-50 rounded-lg border border-orange-100">
                <div className="font-semibold text-orange-800 text-center mb-2">Culture</div>
                <div className="flex flex-wrap gap-1 justify-center">
                  {jobAnalysis.companyInfo.culture.map((trait, index) => (
                    <span key={index} className="text-xs bg-orange-200 text-orange-700 px-2 py-1 rounded-full">
                      {trait}
                    </span>
                  ))}
                </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <p className="text-gray-700">
            This position is classified as a <strong>{jobAnalysis.roleType}</strong> role at the{" "}
            <strong>{formatExperienceLevel(jobAnalysis.experienceLevel).toLowerCase()}</strong> level.
          </p>
        </div>
      </div>
    </div>
